...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for index, letter in enumerate(lowercase):
            try:
                lower_dic[letter] = lowercase[index + shift]
            except IndexError:
                lower_dic[letter] = lowercase[index + shift - 26]
        for index, letter in enumerate(uppercase):
            try:
                upper_dic[letter] = uppercase[index + shift]
            except IndexError:
                upper_dic[letter] = uppercase[index + shift - 26]
        dictionary = {**lower_dic, **upper_dic}
        return dictionary
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        best_words = 0
        current_words = 0
        text = PlaintextMessage(self.message_text, 0)
        for shift in range(0, 27):
            current_words = 0
            text.change_shift(26 - shift)
            current = text.get_message_text_encrypted()
            ...
            if current_words > best_words:
                ...
                best_shift = text.get_shift()
        return (best_shift, best_version)
