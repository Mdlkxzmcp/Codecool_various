...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    updates = 300
    virus_populations = [[] for update in range(updates)]
    for trial in range(numTrials):
        viruses = [SimpleVirus(maxBirthProb, clearProb) for virus in range(numViruses)]
        patient = Patient(viruses, maxPop)
        for update in range(updates):
            virus_populations[update].append(patient.update())
    virus_populations_average = [sum(virus_population) / numTrials for virus_population in virus_populations]
    pylab.plot(virus_populations_average, label="Total Virus Population")
    pylab.title("SimpleVirus")
        ...
        if random.random() < self.getMaxBirthProb() * (1 - popDensity):
            ...
            for resistance in self.getResistances().keys():
                if random.random() < self.getMutProb():
                    if new_resistances[resistance] is True:
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        density = len(current_viruses) / self.getMaxPop()

        for virus in current_viruses:
            try:
                new_viruses.append(virus.reproduce(density, self.getPrescriptions()))
            except NoChildException:
                continue
        self.viruses = current_viruses
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    updates = 300
    virus_populations = [[] for update in range(updates)]
    resistant_virus_populations = [[] for update in range(updates)]
    for trial in range(numTrials):
        ...
        for update in range(updates):
            ...
            resistant_virus_populations[update].append(patient.getResistPop(['guttagonol']))

    virus_populations_average = [sum(virus_population) / numTrials for virus_population in virus_populations]
    resistant_virus_populations_avarage = [
        sum(virus_population) / numTrials for virus_population in resistant_virus_populations]
    pylab.plot(virus_populations_average, label="Average Virus Population")
    pylab.plot(resistant_virus_populations_avarage, label="Average Resistant Population")
