...
def r_squared(y, estimated):
    ...
    residual_sum_of_squares = np.sum(np.array(np.square(np_y - np_estimated)))
    total_sum_of_squares = np.sum(np.array(np.square(np_y - np.mean(np_y))))
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    pylab.plot(x, y, 'bo', label='Sample Points')
    for model in models:
        estimated_values = pylab.polyval(model, x)
        error = r_squared(y, estimated_values)
        pylab.plot(x, estimated_values, 'r-', label='linear regression line')
        pylab.title('Degree: {}\nR square = {:0.4f}'.format(1, error))
        pylab.xlabel('year')
